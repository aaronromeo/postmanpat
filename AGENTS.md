...
- `rulesgen/` — Review Queue web service (SQLite decision store, report-file ingestion, per-lane decision capture, rule/ignore fragment rendering); cobra wiring in `cli/rulesgen.go`, compose service `postmanpat-rulesgen`.
...
- `rulesgen serve` (`--addr` `--reports` `--db` `--fragments` `--poll`): Review Queue web service (default port 8092). Ingests `postmanpat-analyze-*.json` from the reports dir into a SQLite store at startup and every `--poll`; never opens IMAP. Pending clusters dedupe by cluster ID, persist across reports with `last_seen`, refresh count/examples on re-ingestion; `template_lens` never ingested; suppressed-for-both clusters hidden from the queue page (half-suppressed shown with badges). Each lane offers Generate / Decline / Ignore / Snooze controls; generated rules (`rules:` lists) and ignored identities are re-rendered from decisions into `watch.yaml`, `cleanup-onetime.yaml`, `cleanup-ongoing.yaml`, `ignore.yaml` in `--fragments` (re-rendered on decide/revert and on each ingest; `ignore.yaml` is omitted when empty). `GET /healthz` for liveness. Rule/ignore YAML output is byte-parity with `bin/postmanpat-generate-rules.py` (ADR 0003); see CONTEXT.md (rulesgen, Review Queue, Cluster Decision) and ADRs 0003/0004.
