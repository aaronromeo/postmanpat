...
- **Cron wiring (docker/entrypoint.sh).** A new optional environment variable, `POSTMANPAT_ANALYZE_CONFIG`, carries the in-container path of the analyze config. When set, the entrypoint writes a second cron line running analyze **daily at 03:30** (the hourly cleanup tick sits at minute 0) with `--out` pointing at the mounted report directory and `--min-count 1`. When unset, no analyze line is emitted — the crontab is byte-identical to today. The schedule is hardcoded, matching how the cleanup schedule is hardcoded.
- **Compose wiring (docker-compose.yml).** A host directory is mounted into the container at the `--out` location so reports survive the container and are reachable by the future rulesgen service. The live deployment reuses the existing `analyze-out` host directory convention from the one-off workflow.
